...
productgrids.forEach(products=>{
    products.addEventListener('click', function (e) {
    if (e.target.classList.contains('quick-view')) {
        const button = e.target;
        model.classList.add('open');
        moverlay.classList.add('over');
        ...
        modelinfo.innerHTML = `<img class="mimage" src="${productimage}" alt="${producttitle}">
            <div class="mdes"><h4>DESCRIPTION:</h4>${productdes}</div>
            <div class="mprice">${productprice}</div>
            <button id="mbutton">add to cart</button>`;
